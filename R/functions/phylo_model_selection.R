...
phylo_model_selection <- function(tree = tree_list[1], model=full_model,data=full_data_set,family ="binomial"){
      ...
      #Make initial model
        ...
        #Compare model to alternatives
          ...
          for(i in 3:ncol(output)){
            ...
            model_out_i <- pglmm(formula = as.formula(formula_i) ,
                                 data = data,
                                 family = family,
                                 cov_ranef = list(species_i = tree))
            
            
            output[1,i] <- model_out_i$AIC
